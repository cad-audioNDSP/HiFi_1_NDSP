...
int16_t scl_log10_16x16(int16_t x)
{
    ...
    ae_int32x2 X0,X,E,Z,Y,Z2,Z_tmp;
    ae_f32x2 f;
    int e0;
	...
	Z_tmp=AE_SLAI32(Z,8);
    Y=AE_L32_I(ptbl,0);
    Z2=AE_MULFP32X2RAS_L(Z,Z_tmp);
	Z2=AE_SLAI32(Z2,8);
    f=AE_L32_I(ptbl,4);  AE_MULSFP32X2RAS_L(f,Z_tmp,Y); Y=f;
    f=AE_L32_I(ptbl,8);  AE_MULSFP32X2RAS_L(f,Z_tmp,Y); Y=f;
    f=Z;                 AE_MULAFP32X2RAS_L(f,Z2,Y); Y=f;
    /* apply original exponent and scale */
    Z=AE_L32_I(ptbl,24);
	Z=AE_SLAI32(Z,8);
    Y=AE_MULFP32X2RAS_L(Y,Z);
    Z=AE_L32_I(ptbl,28);
    f=Y;AE_MULAFP32X2RAS_L(f,AE_SLAI32(E,24),Z); Y=f;
