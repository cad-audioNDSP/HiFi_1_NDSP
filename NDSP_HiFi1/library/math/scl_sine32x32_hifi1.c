...
*/
    ...
    vsw = AE_MOVDA32(0xc91000); //pi,Q22
    vww = AE_MOVDA32(0xFFFFFFFC);
    vnw = AE_MOVDA32(0x400000); //1<<(22)
    vlw = AE_MOVDA32(0x200); //1<<(9)/4
    vkw = AE_MOVDA32(0x7fc); //1<<(9)-1
    ...
    vdelf = AE_MULFP32X2RAS_L(vaf, vbf);//
    vdw = (vdelf);
    ...
    vxf = (vxc);//cos
    AE_MULSFP32X2RAS_L(vxf, vsf, vdf); //cos - 1/2*sin
    vdf = (vdw);//delta
    AE_MULAFP32X2RAS_L(vsf, vxf, vdf); //y = sin(x0)+cos(x0)dx-1/2*sin(x0)*dx^2
    vrw = (vsf);
    y = AE_MOVAD32_L(vrw);
    return y;
