...
int32_t scl_atan32x32(int32_t x) 
{
    ...
    vnw = AE_MOVDA32(0x1FFFFFFF); //
    vsw = AE_MOVDA32(0xFFFFFFFC); //
    vkw = AE_MOVDA32(0x20000000); //
    vlw = AE_MOVDA32(0x40000000); //
    vzw = AE_MOVI(0);
    vxw = AE_MOVDA32(x);
    ...
    vdf = AE_MULFP32X2RAS_L(vcf, vbf);//d1*d2
    vxf = (vy0);// y0

    vhf = AE_MULF32S_LL(vdf,vxf);//t*y0
    vdf = AE_MULFP32X2RAS_L(vaf, vbf);//d0*d1
    vxf = (vy2);// y2
    AE_MULAF32S_LL(vhf,vdf,vxf);
    vyh = (vhf);
    vyh = AE_SRAI64(vyh, 1);
    vdf = AE_MULFP32X2RAS_L(vaf, vcf);//d0*d2
    vhf = (vyh);
    vxf = (vy1);// y1
    AE_MULSF32S_LL(vhf,vdf,vxf);
    vyh = (vhf);
