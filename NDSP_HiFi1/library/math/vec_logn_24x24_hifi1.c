...
void vec_logn_24x24 (f24 * restrict y,const f24 * restrict x, int N)
{
  ...
  ae_int32x2  vxw, vyw, vnw, vdw, vew,  vsw, vhw, vzw, viw;
  ae_f32x2    vxl;
  ae_f32x2    vxf, vdf;
  ae_f64      vyf;
  ae_int64    vyq;
  ...
  ae_int32x2 vmw_1 = AE_MOVDA32X2(0x7fffff, 0x7fffff);/*ln(2) in Q.31*/
  ae_int32x2 vmw_2 = AE_MOVDA32X2(0x58B90BFC, 0x58B90BFC);/*ln(2) in Q.31*/
  ...
  else
  {
    ...
    const ae_f32   * restrict px = (const ae_f32   *)x;
    for(n=0;n<N;++n)
    {
      vxl = AE_L32_I(px, 0);
      vxl = AE_SRAI32(vxl,8);
