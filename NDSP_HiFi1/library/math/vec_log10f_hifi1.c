  ...
  for (; N>0; N-=blkSize,x+=blkSize,y+=blkSize)
  {
...

    {
      ...
      for ( n=0; n<(blkLen>>1); n++ )
      {
        ...
        AE_L32X2_IP( ex_i, SCR_rd, +2*sz_i32 );

        y0 = ex_f = XT_FLOAT_SX2( ex_i, 2 );

        XT_MADD_SX2( y0, g, fr );
        XT_MADD_SX2( y0, ex_f, _4log10_2minus1.f );
      ...
      if ( blkLen & 1 )
      {
        ...
        ex_i = AE_L32_I( (ae_int32*)SCR_rd, 0 );
