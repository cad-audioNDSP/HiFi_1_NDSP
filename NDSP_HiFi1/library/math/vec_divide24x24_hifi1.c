...
)
{
  ...
  for(n=0; n<(N>>1); n++)
  {
    ...
    /* normalization */
    AE_LA32X2_IP(XX, x_align, px);
  	XX=AE_SRAI32(XX,8);
    AE_LA32X2_IP(YY, y_align, py);
	  YY=AE_SRAI32(YY,8);
    expx = AE_NSAZ32_L(XX);
    expy = AE_NSAZ32_L(YY);
  ...
  if (N&1)
  {
    ...
    _0x400000 = AE_MOVDA32(0x400000);
    /* normalization */
    AE_L32_IP(XX, castxcc(ae_int32, px),0);
	XX=AE_SRAI32(XX,8);
    AE_L32_IP(YY, castxcc(ae_int32, py),0);
	YY=AE_SRAI32(YY,8);
    expx = AE_NSAZ32_L(XX);
    expy = AE_NSAZ32_L(YY);
    XX = AE_SLAA32S(XX, expx);
    YY = AE_SLAA32S(YY, expy);
    exp[0] = (int16_t)(expy - expx + 1);
