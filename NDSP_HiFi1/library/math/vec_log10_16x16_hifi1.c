...
void vec_log10_16x16 (int16_t * restrict y, const int16_t * restrict x, int N)
{
    ...
    for(n=0; n<(N>>1);n++) 
    {
        ...
        Z_lsh16 = AE_TRUNCI16X4F32S(Z,Z,8);
        ae_int16x4 Z2_16ae;
        AE_MULFP16X4RAS_H(Z2_16ae,Z_lsh16,Z_lsh16);

        f=AE_L32_I(ptbl,4);  AE_MULSFP32X16X2RAS_H(f, Y, Z_lsh16); Y=f;
        f=AE_L32_I(ptbl,8);  AE_MULSFP32X16X2RAS_H(f, Y, Z_lsh16); Y=f;
        f=Z;                 AE_MULAFP32X16X2RAS_H(f,Y,Z2_16ae);   Y=f;
        ...
        Z_lsh16 = AE_TRUNCI16X4F32S(Z,Z,8);
	    Y = AE_MULFP32X16X2RAS_H(Y, Z_lsh16);
	
	    Z=AE_L32_I(ptbl,28);
        f=Y;AE_MULAFP32X2RAS(f,AE_SLAI32(E,24),Z); Y=f;
        Y=AE_SLAI32S((Y),4);
        /* save results */
    ...
    if(N&1)
    {
        ...
        Z_lsh16 = AE_TRUNCI16X4F32S(Z,Z,8);
        Z2=AE_MULFP32X16X2RAS_L(Z, Z_lsh16);
        f=AE_L32_I(ptbl,4);  AE_MULSFP32X16X2RAS_H(f, Y, Z_lsh16); Y=f;
        f=AE_L32_I(ptbl,8);  AE_MULSFP32X16X2RAS_H(f, Y, Z_lsh16); Y=f;
        f=Z;                 

        Z20 = AE_SLAI32(Z2, 8);
        AE_MULAFP32X2RAS(f, Z20, Y); Y=f;
	...
	/* apply original exponent and scale */
        ...
        Z20 = AE_SLAI32(Z, 8);
        Y = AE_MULFP32X2RAS(Y, Z20);
        
	    Z=AE_L32_I(ptbl,28);
        f=Y;AE_MULAFP32X2RAS(f,AE_SLAI32(E,24),Z); Y=f;
        Y=AE_SLAI32S(AE_MOVF24X2_FROMINT32X2(Y),4);
        /* save results */
        AE_MOVT32X2(Y,0x800000,lesszero);
