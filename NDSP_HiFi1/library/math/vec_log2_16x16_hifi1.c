...
void vec_log2_16x16 (int16_t * restrict y, const int16_t * restrict x, int N)
{
    ...
    for(n=0; n<(N>>1);n++) 
    {
        ...
        Y=AE_L32_I(ptbl,0);
        ae_int16x4 Z_lsh16 = AE_TRUNCI16X4F32S(Z,Z,8);
        ae_int16x4 Z2_16ae;
        ...
        Z_lsh16 = AE_TRUNCI16X4F32S(Z,Z,8);
	      AE_MULAFP32X16X2RAS_H(f,Y, Z_lsh16);Y=f;
    ...
    if(N&1)
    {
        ...
        /* apply original exponent */
        f=AE_SLAI32(E,18);
	      Z20=AE_SLAI32(Z,8); AE_MULAFP32X2RAS(f,Z20,Y); Y=f;
        Y=AE_SLAI32S(AE_MOVF24X2_FROMINT32X2(Y),2);  
        /* save results */
        AE_MOVT32X2(Y,0x800000,lesszero);
