...
int16_t scl_sqrt16x16(int16_t x )
{
    ...
    sh=AE_NSAZ32_L(X)&~1;
    X=AE_SLAA32(X,sh);
        ae_f16x4   tmp_X16 = AE_MOVF16X4_FROMINT32X2(X);
        tmp_X16 = AE_SEL16_7531(tmp_X16, tmp_X16);
    lezero=AE_LE32(X,0);
    /* compute rsqrt */
    R=polyrsqrtq23[0];
	...
	R_tmp=AE_SLAI32(R,8);
    D=AE_MULFP32X2RAS_L((R_tmp),(R));
    f=0x80000; AE_MULSFP32X2RAS_L(f,(D),(X)); D=f;
    D=AE_MULFP32X2RAS_L((D),(R_tmp));
    D=AE_SLAI32(D,3);
    R=AE_ADD24S(R,D);
    /* compute sqrt and rescale back */
    Y=AE_MULFP32X2RAS_L((X),(R));
