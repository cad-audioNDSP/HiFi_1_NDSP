              ...
              int N)
{
    ...
    while(N>0)
    {
        ...
        for (n=0; n<(M&~1); n+=2)
        {
            ...
            f=_0x400000; AE_MULSFP32X2RAS(f,X,Y_LS); E=f;

            E=AE_SLLI32(E,1);
            f=Y; AE_MULAFP32X2RAS(f,E,Y); Y=f; Y_LS = AE_SLLI32(Y,8);
            f=_0x400000; AE_MULSFP32X2RAS(f,X,Y_LS); E=f;
            E=AE_SLLI32(E,1);
            f=Y; AE_MULAFP32X2RAS(f,E,Y_LS); Y=f; Y_LS = AE_SLLI32(Y,8);
            f=_0x400000; AE_MULSFP32X2RAS(f,X,Y_LS); E=f;
            E=AE_SLLI32(E,1);
            f=Y; AE_MULAFP32X2RAS(f,E,Y_LS); Y=f;

            /* restore original sign */
            X=AE_NEG32(Y);
            AE_MOVT32X2(Y,X,sx);
            Y=AE_SAT24S(Y);
            AE_L32X2_IP(t,(ae_int32x2*)ps,8);
            f = AE_MULFP32X2RAS(Y,t);
