...
int16_t scl_cosine16x16 (int16_t x)
{
    ...
    X=AE_SUB32S(0x800000,AE_ABS32(AE_SUB32(X,0x800000)));
    X=AE_SAT24S(X);
    ae_int32x2 X_LS = AE_SLLI32(X,8);
    /* compute polynomial */
    ae_int32x2 X2_LS=AE_MULFP32X2RAS_L(X,X_LS);
    X2= AE_SLLI32(X2_LS,8);
    Y=costblq23[0];        
    f=costblq23[1];          AE_MULAFP32X2RAS_L(f,Y,X2); Y=f;
    f=costblq23[2];          AE_MULAFP32X2RAS_L(f,Y,X2); Y=f;
    f=AE_SUB32(0x800000,X2_LS); AE_MULAFP32X2RAS_L(f,Y,X2); Y=f;
    /* apply sign */
