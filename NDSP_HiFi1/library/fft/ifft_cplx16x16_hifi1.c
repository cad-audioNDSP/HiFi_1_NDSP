...
static int stage_inner_IDFT4XI2_inpl_merged_ds(int16_t *x, int16_t *y, const int16_t *twd, int N, int stride)
{
    int shift=0;
    ae_int16x4 * restrict px = (ae_int16x4 *)x;
    ae_int16x4 * restrict py = (ae_int16x4 *)y;
    ...
    return shift;
} //stage_inner_IDFT4XI2_inpl_merged_ds
static int stage_first_IDFT4XI2_inpl_ds(int16_t *x, int16_t *twd, int N, int bexp)
{
    ae_int16x4 * restrict px16x4 = (ae_int16x4 *)x;
    ae_int16x4 * restrict py = (ae_int16x4 *)x;
    ae_int32x2 * restrict ptw32x2 = (ae_int32x2 *)twd;
    int stride = N / 4 * sizeof(int16_t)* 2;
    ...
    for (i = 0; i < (N >> 3); i++)
    {
        ...
        ae_int32x2 r1, r2, r3;
        x1 = AE_L16X4_X(px16x4, stride);
        x3 = AE_L16X4_X(px16x4, stride * 3);
        x2 = AE_L16X4_X(px16x4, stride * 2);
        AE_L16X4_XP(x0, px16x4, 8);
    ...
    return 3 - bexp;
} //stage_first_IDFT4XI2_inpl_ds

static int stage_last_IDFT4XI2_inpl_ds(int16_t *x, int16_t *y, int N)
{
    ...
    ae_int16x4 * restrict py10 = (ae_int16x4 *)(y);
    ae_int16x4 * restrict py11 = (ae_int16x4 *)(y + N);
    ae_int16x4 * restrict py12 = (ae_int16x4 *)(y + N / 2);
    ae_int16x4 * restrict py13 = (ae_int16x4 *)(y + 3 * N / 2);
    ...
    for (i = 0; i<(N / 4); i += 2)
    {
        ...
        /* Store the 2 bfly outputs together  */
        AE_S16X4_X(h4, py10, ibitrev);
        AE_S16X4_X(h5, py11, ibitrev);
        AE_S16X4_X(h2, py13, ibitrev);
        AE_S16X4_X(h3, py12, ibitrev); 

        ibitrev = AE_ADDBRBA32(ibitrev, ibitrev_off);
    }
    return shift;
} //stage_last_IDFT4XI2_inpl_ds
...
static int stage_last_DFT2xI2_inpl_ds(int16_t *x, int16_t *y, int N)
{
    ...
    for (i = 0; i<(N / 4); i += 2)
    {
        ...
        AE_S16X4_X(h4, py10, ibitrev);
        AE_S16X4_X(h5, py11, ibitrev);
        AE_S16X4_X(h2, py12, ibitrev);
        AE_S16X4_X(h3, py13, ibitrev); 
...
static int stage_inner_IDFT4XI2_inpl_ref_ds(int16_t *x, int16_t *y, const int16_t *twd, int N, int stride)
{
    int shift;
    ae_int16x4 * restrict px = (ae_int16x4 *)x;
    ae_int16x4 * restrict py = (ae_int16x4 *)y;
...
int ifft_cplx16x16( int16_t* y,int16_t* x,fft_handle_t h,int scalingOption)
{
    ...
    else
    {
        ...
        int shift;
        int bexp = vec_bexp16((const int16_t*)x, 2 * N) - 16;
        ASSERT(bexp >= 0);

        shift = stage_first_IDFT4XI2_inpl_ds(x, (int16_t*)p_twd, N, bexp);
        stride >>= 2;

        if (stride > 4)
        {
            p_twd = (int16_t*)*p++;
            shift += stage_inner_IDFT4XI2_inpl_merged_ds(x, x, p_twd, N, stride);
            stride >>= 2;
            while (stride > 4)
            {
              p_twd = (int16_t*)*p++;
              shift += stage_inner_IDFT4XI2_inpl_ref_ds(x, x, p_twd, N, stride);
              stride >>= 2;
            }
         }
        ...
        if (stride == 4)
        {
            shift += stage_last_IDFT4XI2_inpl_ds(x, y, N);
        }
        else
        {
