...
)
{
  ...
  for (n=0; n<(N>>2); n++)
  {
    AE_LA32X2_IP(vx0p, x_align, px);
    vx0p = AE_SRAI32(vx0p,8);
    AE_LA32X2_IP(vx1p, x_align, px);
    vx1p = AE_SRAI32(vx1p,8);
  ...
  if (n&1)
  {
    ...
    AE_L32_IP(vx0p, castxcc(const ae_f32, px), 0);
    vx0p= AE_SRAI32(vx0p,8);
    vx0f = AE_SLAI32(vx0p, 8);
    vy0f = AE_MULFP32X2RAS_L(vx0f, vcf);
