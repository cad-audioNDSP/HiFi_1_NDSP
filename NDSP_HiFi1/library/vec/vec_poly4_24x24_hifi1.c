...
void vec_poly4_24x24 (f24 * restrict z, const f24 * restrict x, const f24 * restrict c, int lsh, int N )
{
    ...
    ae_f32x2   c0f, c1f, c2f, c3f, c4f, s;
    ae_f32x2 t, vxf_32;
    ...
    z_align = AE_ZALIGN64();
    AE_L32_IP(c0f, pc, sizeof(*pc));
    c0f = AE_SRAI32(c0f, 8);
    AE_L32_IP(c1f, pc, sizeof(*pc));
    c1f = AE_SRAI32(c1f, 8);
    AE_L32_IP(c2f, pc, sizeof(*pc));
    c2f = AE_SRAI32(c2f, 8);
    AE_L32_IP(c3f, pc, sizeof(*pc));
    c3f = AE_SRAI32(c3f, 8);
    AE_L32_IP(c4f, pc, sizeof(*pc));
    c4f = AE_SRAI32(c4f, 8);
    WUR_AE_SAR(lsh+8);
    for (n=0; n<(N&~1); n+=2)
    { 
        AE_LA32X2_IP(vxf_32, x_align, px); 
        t=c3f; AE_MULAFP32X2RAS(t, vxf_32, c4f); s=AE_SAT24S(t);
        t=c2f; AE_MULAFP32X2RAS(t, vxf_32, s  ); s=AE_SAT24S(t);
        t=c1f; AE_MULAFP32X2RAS(t, vxf_32, s  ); s=AE_SAT24S(t);
        t=c0f; AE_MULAFP32X2RAS(t, vxf_32, s  ); 
        t=AE_SLAS32S(t);
    ...
    { 
        ...
        t=AE_SLAS32S(t);
        s=AE_SEL32_HH(t,t);
        AE_S32_L_I(s, (ae_f32 *)pz, 0);//put answer
    }
}
