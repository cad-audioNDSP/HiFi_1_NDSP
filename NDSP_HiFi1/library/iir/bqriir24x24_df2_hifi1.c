                ...
                const int32_t *                x , int N)
{
  ...
  ae_f24x2   st;
  ae_int16x4 cf_g;
  ae_f32x2   cf_a2a1, cf_b2b1, cf_b0;
  ae_int32x2 x_x0x1;
  ae_f64     q0, q1, y0, y1;
  ...
  for ( m=0; m<M-1; m++ )
  {
    ...
    for ( n=0; n<N; n+=2 )
    {
      ...
      ae_int32x2 tmp_t1 = AE_SLAI32S(cf_a2a1,1);
      AE_MULS32_HH(q0,tmp_t1,st);
      AE_MULS32_LL(q0,tmp_t1,st);

      // y0 = b1*st[-1] + b2*st[-2]
      // Q17.46 <- [ Q22*Q23 + 1 ] + [ Q22*Q23 + 1 ]
      //y0 = AE_MULZAAFD24_HH_LL( cf_b2b1, st );
      ae_int32x2 tmp_t2 = AE_SLAI32S(cf_b2b1,1);
      y0 = AE_MUL32_HH(tmp_t2, st);
      AE_MULA32_LL(y0,tmp_t2, st);
      ...
      AE_MULS32_HH(q1, tmp_t1, st);
      AE_MULS32_LL(q1, tmp_t1, st);

      // y1 = b1*st[-1] + b2*st[-2]
      // Q17.46 <- [ Q22*Q23 + 1 ] + [ Q22*Q23 + 1 ]
      //y1 = AE_MULZAAFD24_HH_LL( cf_b2b1, st );
      y1 = AE_MUL32_HH(tmp_t2, st);
      AE_MULA32_LL(y1, tmp_t2, st);
      ...
      ae_int32x2 tmp_t3 = AE_SLAI32S(cf_b0,1);
      AE_MULA32_HL( y0, tmp_t3, st);
      AE_MULA32_LH( y0, tmp_t3, st);

      // y1 += 0*q0 + b0*q1
      // Q17.46 <- Q17.46 + [ Q22*Q23 + 1 ]
      //AE_MULAAFD24_HH_LL( y1, cf_b0, st );
      AE_MULA32_HH( y1, tmp_t3, st);
      AE_MULA32_LL( y1, tmp_t3, st);
