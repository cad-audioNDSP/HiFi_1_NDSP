                  ...
                  const float32_t * x, int N)
{
    ...
    /* Process first 2 channels */ 
    {
        ...
        for(m=0;m<M;m++)
        {
            xtfloatx2 dx0,dy0,dy1,dy2;
            xtfloat a1,a2,b0,b1,b2,dx0_h,dx0_l;
            pZ=(xtfloat *)(z);
            ...
            for (n=0;n<N;n++)
            {
                XT_LSIP(dx0_h,pX,sizeof(float32_t));
                XT_LSIP(dx0_l,pX,2*sizeof(float32_t));
				dx0 = XT_SEL32_HH_SX2(dx0_h, dx0_l);
                dy2=dy1;
                dy1=dy0;
                dy0=dx0;
                XT_MSUB_SX2(dy0,a2,dy2);
                XT_MSUB_SX2(dy0,a1,dy1);
                dx0=XT_MUL_SX2(b2,dy2);
                XT_MADD_SX2(dx0,b1,dy1);
                XT_MADD_SX2(dx0,b0,dy0);
                XT_SSIP(XT_HIGH_S(dx0),pZ,sizeof(float32_t));
                XT_SSIP(XT_LOW_S(dx0),pZ,2*sizeof(float32_t));
			}
	...
	/*Process 3rd channel*/
    {
        ...
        pa1 =( const xtfloat*)(state->cf+3*M);
        pa2 =( const xtfloat*)(state->cf+4*M);
        pDrd=( const xtfloat*)(state->st+2*3*M);
        pDwr=(       xtfloat*)(state->st+2*3*M);
