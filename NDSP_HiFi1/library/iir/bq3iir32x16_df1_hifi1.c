                ...
                const int32_t *                x, int N )
{
  ...
  for ( l=0; l<3; l++ )
  {
    ...
    for ( n=0; n<(N>>1); n++ )
    {
	  ...
	  ae_f32x2 tmp = AE_ROUND32X2F48SASYM(q0,q1);

      #if ( XCHAL_HW_VERSION >= XTENSA_HWVERSION_RI_2022_9 )
	  AE_S32_H_IP(tmp, castxcc(ae_f32, R), 3 * sz_i32);
	  #else
        ae_f32x2 tmp_1 = AE_INTSWAP(tmp);
        AE_S32_L_IP(tmp_1, castxcc(ae_f32, R), 3 * sz_i32);
	  #endif
      AE_S32_L_IP(tmp, castxcc(ae_f32, R), 3 * sz_i32);
