                  ...
                  const float32_t * x, int N)
{
    ...
    M=state->M;
    NASSERT(M>0 && N>0);
    /* Process first 2 channels at a time*/ 
    {
		...
		for (m = 0; m < M; m++)
		{
			...
			xtfloat a1,a2,b0,b1,b2,th,tl;
			pZ=(xtfloat    *)(z);
			/* load delay lines and coefficients */
			XT_LSX2IP(dx0,(xtfloatx2*)pDrd,sizeof(xtfloatx2)); 
			XT_LSX2IP(dx1,(xtfloatx2*)pDrd,sizeof(xtfloatx2)); 
			XT_LSX2IP(dy0,(xtfloatx2*)pDrd,sizeof(xtfloatx2)); 
			XT_LSX2IP(dy1,(xtfloatx2*)pDrd,sizeof(xtfloatx2)); 
			...
			for (n = 0; n < N; n++)
			{
				XT_LSIP(th, pX, (int)sizeof(xtfloat));	
				XT_LSIP(tl, pX, 2*(int)sizeof(xtfloat));	
				xtfloatx2 t = XT_SEL32_HH_SX2(th, tl);
				dz = XT_MUL_SX2(b2, dx1);
				XT_MADD_SX2(dz, b1, dx0);
				XT_MSUB_SX2(dz, a2, dy1);
				XT_MADD_SX2(dz, b0, t);
				XT_MSUB_SX2(dz, a1, dy0);
				dy1 = dy0;
				dy0 = dz;
				dx1 = dx0;
				dx0 = t;
                XT_SSXP(XT_HIGH_S(dz),pZ,(int)sizeof(float32_t));
                XT_SSXP(XT_LOW_S(dz),pZ,2*(int)sizeof(float32_t));
			}
			/* store delay elements */
            XT_SSX2IP(dx0,(xtfloatx2*)pDwr,sizeof(xtfloatx2));
            XT_SSX2IP(dx1,(xtfloatx2*)pDwr,sizeof(xtfloatx2));
            XT_SSX2IP(dy0,(xtfloatx2*)pDwr,sizeof(xtfloatx2));
            XT_SSX2IP(dy1,(xtfloatx2*)pDwr,sizeof(xtfloatx2));
	...
	/*last cahnnel*/
    {
        ...
        pa1 =( const xtfloat*)(state->cf+3*M);
        pa2 =( const xtfloat*)(state->cf+4*M);
        pDrd=( const xtfloat*)(state->st+2*4*M);
        pDwr=(       xtfloat*)(state->st+2*4*M);
        pX  =( const xtfloat*)(x+2);
        for (m=0; m<M; m++)
        {
            ...
            pX=(const xtfloat*)(z+2);/* switch pointer to the input data to the pointer to the output */
        }
    }
    /* final scaling */
	{
		...
		xtfloat* pZ_tmp = (xtfloat*)z;
		pZ = (xtfloat*)z;
		ae_valign al_in = XT_LASX2PP((xtfloatx2 *)pZ_tmp);
		ae_valign al_out = AE_ZALIGN64();
        for (n = 0; n < (3*N)>>1; n++)
        {
            xtfloatx2 t;
            XT_LASX2IP(t, al_in, (xtfloatx2*)pZ_tmp);
            t = XT_MUL_SX2(t, scale2);
            XT_SASX2IP(t, al_out, (xtfloatx2*)pZ);
        }
