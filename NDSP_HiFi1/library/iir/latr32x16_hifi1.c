                                   ...
                                   int16_t            scale )
{
  ...
  if ( procFxn == latrX_proc )
  {
    ...
    ae_int16x4 *pK;
    ae_int16x4 K0, KT0;
    ae_int32x2 coef32x2D;
    ae_int32x2 *coef32x2P;
    ae_valign aK0;
    coef32x2P = (ae_int32x2*)coef32;
    pK = (ae_int16x4*)(k+M-1);
    aK0 = AE_LA64_PP(pK);
    for ( m=0; m<(M>>2); m++ )
    {
      AE_LA16X4_RIP(K0,aK0,pK); //K0 contains the elements in normal order(as seen in binary)
      KT0 = AE_SEL16_5432(K0,K0);
      coef32x2D = AE_MOVINT32X2_FROMINT16X4(K0);
      AE_S32X2_IP(coef32x2D, coef32x2P, sizeof(ae_int32x2));
    }
    if ( M & 2 )
    {
       ae_int16x4 K1;
       AE_L16_IP(K0,(ae_int16*)pK,-sizeof(ae_int16));
       AE_L16_IP(K1,(ae_int16*)pK,-sizeof(ae_int16));
       K0 = AE_SEL16_7362(K0,K1); //K0,K1 any one element each
       coef32x2D = AE_MOVINT32X2_FROMINT16X4(K0);
       #if ( XCHAL_HW_VERSION >= XTENSA_HWVERSION_RI_2022_9)       
	  AE_S32_H_IP(coef32x2D,(ae_int32*)coef32x2P,sizeof(ae_int32));
       #else
	 coef32x2D = AE_INTSWAP(coef32x2D);
	 AE_S32_L_IP(coef32x2D,(ae_int32*)coef32x2P,sizeof(ae_int32));
       #endif
     }
    if ( M & 1 )
    {
      K0 = AE_L16_I((ae_int16*)pK, 0);
      coef32x2D = AE_MOVINT32X2_FROMINT16X4(K0);
      coef32x2D = AE_SLAI32(coef32x2D,16);
      AE_S32_L_IP(coef32x2D,(ae_int32*)coef32x2P,sizeof(ae_int32));
    }
    if(((M4 - M)>>1)) //M4 always> M.  at M=9,10, 13,14, 17,18... last 2 16 bit elements to be made 0
    {
    	coef32x2D= AE_ZERO32();
    	AE_S32_L_IP(coef32x2D,(ae_int32*)coef32x2P,0);
    }
  }
  else
  {
                  ...
                  int16_t scale, int N, int M )   // scale    [in    ] Q15
{
  ...
  for ( n=0; n<N; n++ )
  {
    ...
    ae_f32x2 tmp = AE_ROUND32X2F48SASYM(q2, q2);
    AE_S32_L_IP(tmp, (ae_int32*)R, sizeof(int32_t));
  }
  //
  // Store updated delay line elements.
  //
