...

{
  ...
  int         K, offa, offb;
  ae_int32x2  vpw;
  ae_int32x2    vcl, vdl, zl;
  ae_int32x2  vxl, val, vbl;
  ae_int32 *    Z1;
  ae_int32 *    Z2;
  ae_int32 *    T1;
  ae_int32 *    T2;
  ...
  for(p=0; p<P; p+=2)
    {
      ...
      for(n=0; n<(N>>1); ++n)
        ...
        { 
          ...
          AE_L32_IP(val,T1, 4);
          AE_L32_IP(vbl,T1, 0);
          AE_S32_L_IP(val, castxcc(ae_int32, Z1), 4);
          AE_S32_L_IP(vbl, castxcc(ae_int32, Z2), 4);
          AE_S32_L_IP(zl,castxcc(ae_int32,Z1),0);
          AE_S32_L_IP(zl, castxcc(ae_int32, Z2), 0);
        }
        for(m=0; m<M; m++)
        {
          ae_f64 vah,vbh;
          ...
          vah = (vha);
          vbh = (vhb);
          vy1 = AE_MOVINT32X2_FROMF24X2(AE_ROUNDSP24Q48ASYM(vah));
          vy2 = AE_MOVINT32X2_FROMF24X2(AE_ROUNDSP24Q48ASYM(vbh));
          AE_L32_IP(vpw, pz, 4); 
