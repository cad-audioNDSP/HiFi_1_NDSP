...

{
  ...
  ae_int32x2  vpw;
  ae_int64    zz;
  ae_f32x2    vxl,val,vbl,vyl;

  ae_f32x2 * restrict T1;
  ae_f32x2 * restrict T2;
  ae_f32x2 * restrict Y1;
  ae_f32x2 * restrict Y2;
  ...
  for (m=0; m<4; m++)
  {
    ...
    px = (const ae_f32x2 *)x+m*N;
    px2 = (ae_f32x2 *)((const ae_f32 *)px+N);
    ...
    for(n=0; n<(N>>1); n++)
    {
      ...
      AE_L32X2_IP(val, castxcc(ae_f32x2, T1), 0);
      AE_L32X2_IP(vbl, castxcc(ae_f32x2, T2), 0);
      cc = AE_SEL32_HH(val,vbl);
      b0 = AE_SEL32_LL(val,vbl);

      
      AE_MULAF32R_HH(vah, cc, vxl) ;
      AE_MULAF32R_LL(vah, cc, vxl) ; 
     
      AE_MULAF32R_HH(vch, cc, vyl) ;
      AE_MULAF32R_LL(vch, cc, vyl) ; 
      
      AE_MULAF32R_HH(vbh, b0, vxl) ;
      AE_MULAF32R_LL(vbh, b0, vxl) ; 
      
      AE_MULAF32R_HH(vdh, b0, vyl) ;
      AE_MULAF32R_LL(vdh, b0, vyl) ; 
    } 
