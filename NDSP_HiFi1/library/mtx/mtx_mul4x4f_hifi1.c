...
void mtx_mul4x4f(float32_t * restrict z, const float32_t *restrict x, const float32_t *restrict y, int L)
{
  ...
  //const xtfloatx2    *  restrict py_tmp=(const xtfloatx2      *)x;
  int l, m;
  ...
  for (l = 0; l<L; l++, y += 4 * 4)
  {
    ...
    { 
      ...
      XT_MADDMUX_S(A0,vx0_temp,vy2,0);
      XT_MADDMUX_S(A1,vx0_temp,vy3,0);
 
     
      XT_LASX2IP(vx1, x_align, px);
      XT_MADDMUX_S(A0,vx1,vy4,0);
      XT_MADDMUX_S(A1,vx1,vy5,0);
      vx1_temp=XT_SEL32_LH_SX2(vx1,vx1);
      XT_MADDMUX_S(A0,vx1_temp,vy6,0);
      XT_MADDMUX_S(A1,vx1_temp,vy7,0);
