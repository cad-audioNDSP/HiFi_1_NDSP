                          ...
                          int wrIx, int D, int N, int M )
{
  ...
  for ( n=0; n<(N>>2); n++ )
  {
    ...
    for (d=0;d<D;d++)
    {
      ...
      ae_int32x2 res1 = AE_ROUND32X2F48SASYM(q0,q1);
      ae_int32x2 res2 = AE_ROUND32X2F48SASYM(q2,q3);
	  #if ( XCHAL_HW_VERSION >= XTENSA_HWVERSION_RI_2022_9 )
        AE_S32_H_XP( res1, Y, +D*4 );
        AE_S32_L_XP( res1, Y, +D*4 );
        AE_S32_H_XP( res2, Y, +D*4 );
        AE_S32_L_XP( res2, Y, 0 );			  //  Code for RI.9 and later HW
      #else
        AE_S32_L_XP( AE_SEL32_HH(res1,res1), Y, +D*4 );
        AE_S32_L_XP( res1, Y, +D*4 );
        AE_S32_L_XP( AE_SEL32_HH(res2,res2), Y, +D*4 );
        AE_S32_L_XP( res2, Y, 0 );
      #endif
    }
  }
