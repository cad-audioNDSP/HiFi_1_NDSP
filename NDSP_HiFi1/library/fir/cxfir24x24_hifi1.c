...
void cxfir24x24_process( cxfir24x24_handle_t handle, complex_fract32 * restrict y, const complex_fract32 * restrict x , int N)
{
  ...
  for ( n=0; n<(N>>2); n++ )
  {
    ...
    // Q23 <- Q(23+8) - 8
    AE_L32X2_IP( d0ri, X, +8 );
    AE_L32X2_IP( d1ri, X, +8 );
    AE_L32X2_IP( d2ri, X, +8 );
    AE_L32X2_IP( d3ri, X, +8 );

    // Store 4 complex samples to the delay line buffer with circular address
    // update.
    // Q(23+8) <- Q23 + 8
    AE_S32X2_XC( d0ri, D_wr, +8 );
    AE_S32X2_XC( d1ri, D_wr, +8 );
    AE_S32X2_XC( d2ri, D_wr, +8 );
    AE_S32X2_XC( d3ri, D_wr, +8 );
    ...
    // Q23 <- Q(23+8) - 8
    AE_L32X2_XC( d0ri, D_rd, +8 );
    d0ri = AE_SRAI32(d0ri,8);
    AE_L32X2_XC( d1ri, D_rd, +8 );
    d1ri = AE_SRAI32(d1ri,8);
    AE_L32X2_XC( d2ri, D_rd, +8 );
    d2ri = AE_SRAI32(d2ri,8);
    ...
    // 5 AE registers.
    // Q23 <- Q(23+8) - 8
    AE_L32X2_XC( d3ri, D_rd, +8 );
    d3ri = AE_SRAI32(d3ri,8);
    AE_L32X2_XC( d4ri, D_rd, +8 );
    d4ri = AE_SRAI32(d4ri,8);
    ...
    // Q23 <- Q(23+8) - 8
    AE_L32X2_IP( c0ri, C, +8 );
    c0ri = AE_SRAI32(c0ri,8);
    AE_L32X2_IP( c1ri, C, +8 );
    c1ri = AE_SRAI32(c1ri,8);
    ...
    q0r = AE_MUL32_HH( d0ri, c0ri ); // re*re-im*im
    AE_MULS32_LL( q0r,d0ri, c0ri ); // re*re-im*im
    q1r = AE_MUL32_HH( d1ri, c0ri ); // re*re-im*im
    AE_MULS32_LL( q1r, d1ri, c0ri ); // re*re-im*im

    // Q16.47 <- Q16.47 + [ Q23*Q23 + 1 ] + [ Q23*Q23 + 1 ]
    //q0i = AE_MULZAAFD24_HL_LH( d0ri, c0ri ); // re*im+im*re
    //q1i = AE_MULZAAFD24_HL_LH( d1ri, c0ri ); // re*im+im*re
    
    q0i = AE_MUL32_HL( d0ri, c0ri ); // re*re-im*im
    AE_MULA32_LH( q0i,d0ri, c0ri ); // re*re-im*im
    q1i = AE_MUL32_HL( d1ri, c0ri ); // re*re-im*im
    AE_MULA32_LH( q1i, d1ri, c0ri ); // re*re-im*im

    // Q16.47 <- Q16.47 + [ Q23*Q23 + 1 ] + [ Q23*Q23 + 1 ]
    //q2r = AE_MULZASFD24_HH_LL( d2ri, c0ri ); // re*re-im*im
    //q3r = AE_MULZASFD24_HH_LL( d3ri, c0ri ); // re*re-im*im
    q2r = AE_MUL32_HH( d2ri, c0ri ); // re*re-im*im
    AE_MULS32_LL( q2r,d2ri, c0ri ); // re*re-im*im
    q3r = AE_MUL32_HH( d3ri, c0ri ); // re*re-im*im
    AE_MULS32_LL( q3r, d3ri, c0ri ); // re*re-im*im

    // Q16.47 <- Q16.47 + [ Q23*Q23 + 1 ] + [ Q23*Q23 + 1 ]
    //q2i = AE_MULZAAFD24_HL_LH( d2ri, c0ri ); // re*im+im*re
    //q3i = AE_MULZAAFD24_HL_LH( d3ri, c0ri ); // re*im+im*re
    
    q2i = AE_MUL32_HL( d2ri, c0ri ); // re*re-im*im
    AE_MULA32_LH( q2i,d2ri, c0ri ); // re*re-im*im
    q3i = AE_MUL32_HL( d3ri, c0ri ); // re*re-im*im
    AE_MULA32_LH( q3i, d3ri, c0ri ); // re*re-im*im
                                                           
    // Q16.47 <- Q16.47 + [ Q23*Q23 + 1 ] + [ Q23*Q23 + 1 ]
    //AE_MULASFD24_HH_LL( q0r, d1ri, c1ri ); // re*re-im*im
    //AE_MULASFD24_HH_LL( q1r, d2ri, c1ri ); // re*re-im*im
    AE_MULA32_HH( q0r, d1ri, c1ri ); // re*re-im*im
    AE_MULS32_LL( q0r,d1ri, c1ri ); // re*re-im*im
    AE_MULA32_HH(q1r, d2ri, c1ri ); // re*re-im*im
    AE_MULS32_LL( q1r, d2ri, c1ri ); // re*re-im*im

    // Q16.47 <- Q16.47 + [ Q23*Q23 + 1 ] + [ Q23*Q23 + 1 ]
    //AE_MULAAFD24_HL_LH( q0i, d1ri, c1ri ); // re*im+im*re
    //AE_MULAAFD24_HL_LH( q1i, d2ri, c1ri ); // re*im+im*re
    AE_MULA32_HL(q0i, d1ri, c1ri ); // re*re-im*im
    AE_MULA32_LH( q0i,d1ri, c1ri ); // re*re-im*im
    AE_MULA32_HL(q1i,  d2ri, c1ri ); // re*re-im*im
    AE_MULA32_LH( q1i, d2ri, c1ri ); // re*re-im*im
                                                           
    // Q16.47 <- Q16.47 + [ Q23*Q23 + 1 ] + [ Q23*Q23 + 1 ]
    //AE_MULASFD24_HH_LL( q2r, d3ri, c1ri ); // re*re-im*im
    //AE_MULASFD24_HH_LL( q3r, d4ri, c1ri ); // re*re-im*im
    AE_MULA32_HH(q2r, d3ri, c1ri ); // re*re-im*im
    AE_MULS32_LL( q2r,d3ri, c1ri ); // re*re-im*im
    AE_MULA32_HH(q3r, d4ri, c1ri ); // re*re-im*im
    AE_MULS32_LL( q3r, d4ri, c1ri ); // re*re-im*im

    // Q16.47 <- Q16.47 + [ Q23*Q23 + 1 ] + [ Q23*Q23 + 1 ]
    //AE_MULAAFD24_HL_LH( q2i, d3ri, c1ri ); // re*im+im*re
    //AE_MULAAFD24_HL_LH( q3i, d4ri, c1ri ); // re*im+im*re
    AE_MULA32_HL(q2i, d3ri, c1ri ); // re*re-im*im
    AE_MULA32_LH( q2i,d3ri, c1ri ); // re*re-im*im
    AE_MULA32_HL( q3i, d4ri, c1ri ); // re*re-im*im
    AE_MULA32_LH( q3i, d4ri, c1ri ); // re*re-im*im
    ...
    for ( m=0; m<(M>>1)-1; m++ )
    {
      ...
      AE_MULA32_HH( q0r, d0ri, c0ri ); // re*re-im*im
      AE_MULS32_LL( q0r,d0ri, c0ri ); // re*re-im*im
      AE_MULA32_HH(q1r, d1ri, c0ri ); // re*re-im*im
      AE_MULS32_LL( q1r, d1ri, c0ri ); // re*re-im*im

      // Q16.47 <- Q16.47 + [ Q23*Q23 + 1 ] + [ Q23*Q23 + 1 ]
      //AE_MULAAFD24_HL_LH( q0i, d0ri, c0ri ); // re*im+im*re
      //AE_MULAAFD24_HL_LH( q1i, d1ri, c0ri ); // re*im+im*re
      AE_MULA32_HL(q0i, d0ri, c0ri ); // re*re-im*im
      AE_MULA32_LH( q0i, d0ri, c0ri ); // re*re-im*im
      AE_MULA32_HL(q1i,  d1ri, c0ri ); // re*re-im*im
      AE_MULA32_LH( q1i, d1ri, c0ri ); // re*re-im*im

      // Q16.47 <- Q16.47 + [ Q23*Q23 + 1 ] + [ Q23*Q23 + 1 ]
      //AE_MULASFD24_HH_LL( q2r, d2ri, c0ri ); // re*re-im*im
      //AE_MULASFD24_HH_LL( q3r, d3ri, c0ri ); // re*re-im*im
      AE_MULA32_HH(q2r, d2ri, c0ri ); // re*re-im*im
      AE_MULS32_LL( q2r,d2ri, c0ri ); // re*re-im*im
      AE_MULA32_HH( q3r, d3ri, c0ri ); // re*re-im*im
      AE_MULS32_LL( q3r, d3ri, c0ri ); // re*re-im*im

      // Q16.47 <- Q16.47 + [ Q23*Q23 + 1 ] + [ Q23*Q23 + 1 ]
      //AE_MULAAFD24_HL_LH( q2i, d2ri, c0ri ); // re*im+im*re
      //AE_MULAAFD24_HL_LH( q3i, d3ri, c0ri ); // re*im+im*re
      AE_MULA32_HL(q2i, d2ri, c0ri ); // re*re-im*im
      AE_MULA32_LH( q2i,d2ri, c0ri ); // re*re-im*im
      AE_MULA32_HL(q3i, d3ri, c0ri ); // re*re-im*im
      AE_MULA32_LH( q3i, d3ri, c0ri ); // re*re-im*im
    ...
    q0r = AE_SLAI64S(q0r,1);
    q0i = AE_SLAI64S(q0i,1);
    q1r = AE_SLAI64S(q1r,1);
    q1i = AE_SLAI64S(q1i,1);
    q2r = AE_SLAI64S(q2r,1);
    q2i = AE_SLAI64S(q2i,1);
    q3r = AE_SLAI64S(q3r,1);
    q3i = AE_SLAI64S(q3i,1);
    ...
    // 2xQ(23+8) <- 2xQ23 + 8
    d0ri = AE_SLAI32(d0ri,8);
    AE_SA32X2_IP( d0ri, Y_va, Y );
    d1ri = AE_SLAI32(d1ri,8);
    AE_SA32X2_IP( d1ri, Y_va, Y );
    d2ri = AE_SLAI32(d2ri,8);
    AE_SA32X2_IP( d2ri, Y_va, Y );
    d3ri = AE_SLAI32(d3ri,8);
    AE_SA32X2_IP( d3ri, Y_va, Y );
  }
