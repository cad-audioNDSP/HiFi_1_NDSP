...
DISCARD_FUN(firdecf_handle_t, firdecf_init, (void * objmem, int D, int M, const float32_t * restrict h))
DISCARD_FUN(void, firdecf_process, (firdecf_handle_t _firdec, float32_t * restrict y, const float32_t * x, int N))
...
size_t firdecf_alloc( int D, int M )
{
  ...
  M=(M+1)&~1;
  L =(M+8*D + 3) & ~3;
  return sizeof(firdecf_t)+(M+L)*sizeof(float32_t)+7;
                               ...
                               int M, const float32_t * restrict h )
{
    ...
    /* partition the memblock */
    ptr     = objmem;
    pd = (float32_t*)((((uintptr_t)ptr)+7)&~7);
    ph = pd+L;
    firdec=(firdecf_t*)(ph+M1);
    firdec->M   = M1;
    firdec->D   = D;
    firdec->delLen=L;
    firdec->h = ph;
    ...
    for (m=0; m<M; m++) ph[m]=h[m];
    for (;m<M1; m++) ph[m]=0.f;
    for (m=0; m<L; m++) pd[m]=0.f;
    return firdec;

} // firdecf_init()

typedef float32_t* (*tfun)(
    const float32_t* h, int M, 
    float32_t* d, float32_t* p, int L,
    float32_t * y, const float32_t * x, int N, int D );
...
void firdecf_process( firdecf_handle_t _firdec, 
                    ...
                    const float32_t *                x, int N )
{
    static const tfun fxns[]={&firdecf_D2,&firdecf_D3,&firdecf_D4};
    firdecf_t *state;
    int M,D,L;
    tfun fun;
