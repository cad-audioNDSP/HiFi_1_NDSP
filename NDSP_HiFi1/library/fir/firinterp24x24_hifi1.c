    ...
    const f24 * restrict      x, int N)
{
    ...
    int q_frac_int;
    ae_p24x2s g_frac;
    ae_f32x2 X01, X23, X12, X34, H01, H23, t;
    ae_valign aX0, aX1;
    ae_f64 q0, q1, q2, q3;
    ae_f32x2 d0, d1, d2, d3;
    ...
    for (n = 0; n < N; n += 4)
    {
        ...
        for (j = 0; j < D; j++)
        {
            ...
            X12 = AE_MOVF32X2_FROMINT32X2(AE_INTSWAP(X12));
            X34 = AE_MOVF32X2_FROMINT32X2(AE_INTSWAP(X34));
            ...
            d0 = AE_ROUND24X2F48SASYM(q1,q0);
            d0 = AE_SLAI32(d0, 8);
            AE_S32_L_XP(d0, (ae_int32 *)pY, 4 * D);
            d1 = AE_INTSWAP(d0);
            AE_S32_L_XP(d1, (ae_int32 *)pY, 4 * D);
            d2 = AE_ROUND24X2F48SASYM(q3, q2);
            d2 = AE_SLAI32(d2, 8);
            AE_S32_L_XP(d2, (ae_int32 *)pY, 4 * D);
            d3 = AE_INTSWAP(d2);
            AE_S32_L_XP(d3, (ae_int32 *)pY, (1 - 3 * D) * 4);
        }
        /* update delay line */
