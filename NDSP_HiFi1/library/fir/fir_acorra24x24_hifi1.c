                ...
                int N )
{
    ...
    s_ptr = s;
    x_buf = (ae_f32*)ALIGNED_ADDR( s_ptr, 8 );
    s_ptr = x_buf + 2*N + 3;
  ...
  #pragma concurrent
    ...
    for ( n=0; n<((N+1)>>1); n++ )
    {
        AE_LA32X2_IP( x0, S_va, S );
        AE_S32X2_IP( x0, D, +8 );
    }
    // Second copy of x[N].
    S = (const ae_f32x2 *)x;
    D = (      ae_f32x2 *)( (f24 *)D - ( N & 1 ) );
    S_va = AE_LA64_PP( S );
    D_va = AE_ZALIGN64();
#ifdef COMPILER_XTENSA
  #pragma concurrent
    ...
    D_s = (ae_f32 *)( (f24 *)D - ( N & 1 ) );
    z_f24x2 = 0;
    for ( n=0; n<3; n++ )
    {
        AE_S32_L_IP( z_f24x2, D_s, +4 );
    }
  ...
  M=(N+1)&~1;
    ...
    for ( n=0; n<(N&~3); n+=4 )
    {
        ...
        pX0=(const ae_f32x2*)(x_buf+n);
        pX1=(const ae_f32x2*)(x_buf+n+1);
        ax0=AE_LA64_PP(pX1);
        ay=AE_LA64_PP(pY);
        ...
        for ( m=0; m<M; m+=2 )
        {
            AE_LA32X2_IP(Y01,ay,pY);
            //AE_MULAAFD24_HH_LL(a0,Y01,X01);
            AE_MULAF32R_HH(a0, Y01, X01);
            AE_MULAF32R_LL(a0, Y01, X01);
            //AE_MULAAFD24_HH_LL(a1,Y01,X12);
            AE_MULAF32R_HH(a1, Y01, X12);
            AE_MULAF32R_LL(a1, Y01, X12);
            AE_L32X2_IP (X01,pX0,sizeof(int32_t)*2);
            AE_LA32X2_IP(X12,ax0,pX1);
            //AE_MULAAFD24_HH_LL(a2,Y01,X01);
            AE_MULAF32R_HH(a2, Y01, X01);
            AE_MULAF32R_LL(a2, Y01, X01);
            //AE_MULAAFD24_HH_LL(a3,Y01,X12);
            AE_MULAF32R_HH(a3, Y01, X12);
            AE_MULAF32R_LL(a3, Y01, X12);
        }
        d0 = AE_ROUND24X2F48SASYM( a0, a1 ); AE_SA32X2_IP( AE_SLAI32S(d0, 8), ar,pR);
        d0 = AE_ROUND24X2F48SASYM( a2, a3 ); AE_SA32X2_IP( AE_SLAI32S(d0, 8), ar,pR);
    }
    AE_SA64POS_FP(ar,pR);
    ...
    if(N)
    {
        ...
        for ( m=0; m<M; m+=2 )
        {
            ...
            Y01 = AE_SRAI32(Y01, 8);
            //AE_MULAAFD24_HH_LL(a0,Y01,X01);
            AE_MULAF32S_HH(a0, Y01, X01);
            AE_MULAF32S_LL(a0, Y01, X01);
            //AE_MULAAFD24_HH_LL(a1,Y01,X12);
            AE_MULAF32S_HH(a1, Y01, X12);
            AE_MULAF32S_LL(a1, Y01, X12);
            AE_L32X2_IP (X01,pX0,sizeof(int32_t)*2);
            X01 = AE_SRAI32(X01, 8);
            AE_LA32X2_IP(X12,ax0,pX1);
            X12 = AE_SRAI32(X12, 8);
            //AE_MULAAFD24_HH_LL(a2,Y01,X01);
            AE_MULAF32S_HH(a2, Y01, X01);
            AE_MULAF32S_LL(a2, Y01, X01);
            //AE_MULAAFD24_HH_LL(a3,Y01,X12);
            AE_MULAF32S_HH(a3, Y01, X12);
            AE_MULAF32S_LL(a3, Y01, X12);
        }
        d0 = AE_ROUND24X2F48SASYM( a0, a1 ); 
		#if ( XCHAL_HW_VERSION >= XTENSA_HWVERSION_RI_2022_9 )
          AE_S32_H_IP(AE_SLAI32S(d0,8), castxcc(ae_f32, pR), 4);
 		#else
          AE_S32_L_IP(AE_SLAI32S(AE_INTSWAP(d0),8), castxcc(ae_f32, pR), 4);
		#endif


        if (N>1) AE_S32_L_IP(AE_SLAI32S(d0,8), castxcc(ae_f32, pR), 4);
        d0 = AE_ROUND24X2F48SASYM( a2, a3 );
		#if ( XCHAL_HW_VERSION >= XTENSA_HWVERSION_RI_2022_9 )
          if (N>2) AE_S32_H_IP(AE_SLAI32S(d0,8), castxcc(ae_f32, pR), 4);
		#else
          if (N>2) AE_S32_L_IP(AE_SLAI32S(AE_INTSWAP(d0),8), castxcc(ae_f32, pR), 4);
		#endif
