              ...
              int N, int M )
{
        ...
        ae_f32x2   * restrict pe = (      ae_f32x2 *)e;
        ae_f32x2   * restrict ph = (      ae_f32x2 *)h;
  const ae_f32x2   * restrict px = (const ae_f32x2 *)x;
  const ae_f32x2   * restrict pr = (const ae_f32x2 *)r;
  const ae_f32x2   * restrict px1;  
  const ae_f32x2   * restrict ph1;  
  ...
  */
    for ( n=0; n<N; n+=4 )
    {
        ae_f32x2 X23,X34,H01,H23;
        ae_f32x2 R01,R23,E01,E23;
        ae_f32x2 x0,x1;
        ae_valign ax;

        ph = (ae_f32x2 *)(h+M-2);
        px = (const ae_f32x2 *)(x+n);
        px1= (const ae_f32x2 *)(x+n+1);
        ax = AE_LA64_PP(px1);

        A2 = A3 = AE_ZERO();
        AE_L32X2_RIP(H01,ph);
        AE_L32X2_IP (X23,px,8);
        AE_LA32X2_IP(X34,ax,px1);
        A0 = AE_MULF32R_HH(X23,H01);
        AE_MULAF32R_LL(A0,X23,H01);
        //A0=AE_MULZAAFD24_HH_LL(X23,H01);
        A1 = AE_MULF32R_HH(X34,H01);
        AE_MULAF32R_LL(A1,X34,H01);
        ...
        for ( m=0; m<M-2; m+=2 )
        {
            ...
            AE_L32X2_IP (X23,px,8);
            AE_LA32X2_IP(X34,ax,px1);
            AE_MULAF32R_HH(A0, X23,H23);
            AE_MULAF32R_LL(A0, X23,H23);
            //AE_MULAAFD24_HH_LL(A0,X23,H23);
            AE_MULAF32R_HH(A1,X34,H23);
            AE_MULAF32R_LL(A1,X34,H23);
            //AE_MULAAFD24_HH_LL(A1,X34,H23);
            AE_MULAF32R_HH(A2,X23,H01);
            AE_MULAF32R_LL(A2,X23,H01);
            //AE_MULAAFD24_HH_LL(A2,X23,H01);
            AE_MULAF32R_HH(A3,X34,H01);
            AE_MULAF32R_LL(A3,X34,H01);
        ...
        AE_L32X2_IP (R01,pr,2*sizeof(int32_t));
        R01 = AE_SRAI32(R01,8);
        AE_L32X2_IP (R23,pr,2*sizeof(int32_t));
        R23 = AE_SRAI32(R23,8);
        E01=AE_SAT24S(AE_SUB32S(R01,x0));
        E23=AE_SAT24S(AE_SUB32S(R23,x1));
          
        E01 = AE_SLAI32(E01,8);
        AE_S32X2_IP (E01,pe,2*sizeof(int32_t));
        E23= AE_SLAI32(E23,8);
        AE_S32X2_IP (E23,pe,2*sizeof(int32_t));
    }
    ...
    */
    ...
    for ( m=0; m<M; m+=4 )
    {
        int n;
        ae_valign ax;
        ae_f32x2 E01,E23,X01,X12,X23,X34;
        ae_f32x2 x0,x1,x4,x5;
        ae_f32x2    val;
        pe=(      ae_f32x2*)e;
        px=(const ae_f32x2*)(x+m);
        px1=(const ae_f32x2*)(x+m+1);
        ax=AE_LA64_PP(px1);
        AE_L32X2_IP(E23,pe,2*sizeof(int32_t));
        AE_L32X2_IP (X01,px,2*sizeof(int32_t));
        AE_LA32X2_IP(X12,ax,px1);
        A2=A3=AE_ZERO();
        A0=AE_MULF32R_HH(X01,E23);
        AE_MULAF32R_LL(A0,X01,E23);
        //A0=AE_MULZAAFD24_HH_LL(X01,E23);
        A1=AE_MULF32R_HH(X12,E23);
        AE_MULAF32R_LL(A1,X12,E23);
        //A1=AE_MULZAAFD24_HH_LL(X12,E23);
        for ( n=0; n<N-2; n+=2 )
        {
            E01=E23;
            AE_L32X2_IP(E23,pe,2*sizeof(int32_t));
            AE_L32X2_IP (X23,px,2*sizeof(int32_t));
            AE_LA32X2_IP(X34,ax,px1);
            AE_MULAF32R_HH(A0, X23,E23);
            AE_MULAF32R_LL(A0, X23,E23);
            //AE_MULAAFD24_HH_LL(A0,X23,E23);
            AE_MULAF32R_HH(A1, X34,E23);
            AE_MULAF32R_LL(A1, X34,E23);
            //AE_MULAAFD24_HH_LL(A1,X34,E23);
            AE_MULAF32R_HH(A2, X23,E01);
            AE_MULAF32R_LL(A2, X23,E01);
            //AE_MULAAFD24_HH_LL(A2,X23,E01);
            AE_MULAF32R_HH(A3, X34,E01);
            AE_MULAF32R_LL(A3, X34,E01);
        ...
        if ( s_exp < 15 )
        {
          ...
          x0 = (AE_TRUNCA32X2F64S(A0,A1,24));
          x1 = (AE_TRUNCA32X2F64S(A2,A3,24));
        }
        else
        {
          x0 = (AE_SLAA32S( AE_TRUNCA32X2F64S(A0,A1,32),s_exp - 15 - 8));
          x1 = (AE_SLAA32S( AE_TRUNCA32X2F64S(A2,A3,32),s_exp - 15 - 8));
        }
        ph1 = (ae_f32x2 *) ph;
        AE_L32X2_RIP(x4, ph1);
        x4 = AE_SRAI32(x4,8);
        ...
        x0 = AE_SAT24S(AE_ADD32S(x0,x4));
        x1 = AE_SAT24S(AE_ADD32S(x1,x5));
        x0 = AE_SLAI32(x0,8);
        AE_S32X2_RIP(x0,ph); 
        x1 = AE_SLAI32(x1,8);
        AE_S32X2_RIP(x1,ph);
