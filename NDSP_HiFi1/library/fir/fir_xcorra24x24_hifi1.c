                ...
                int N, int M )
{
    ...
    for ( m=0; m<((M+1)>>1); m++ )
    {
        AE_LA32X2_IP( y0, S_va, S );
        AE_S32X2_IP( y0, D, +8 );
    }
    // Append three zeros to allow for block-4 processing.
    D = (      ae_f32x2 *)(y_buf+M);
    AE_S32_L_I( AE_ZERO24(), (      ae_f32 *)D, 0*sizeof(int32_t) );
    AE_S32_L_I( AE_ZERO24(), (      ae_f32 *)D, 1*sizeof(int32_t) );
    AE_S32_L_I( AE_ZERO24(), (      ae_f32 *)D, 2*sizeof(int32_t) );
